...
{
    // public static event Action SwapTurn;
    private int id = 0;
    ...
    public void Attack1()
    {
        if ((turn)&&(life))
        {
            txt.text = "Каспер пытается удалить вирус с устройства!";
    ...
    public void Attack2()
    {
        if ((turn) && (life))
        {
            txt.text = "Каспер лечит битые файлы!";
            hp += 15 * lvl * dd * complexity;
            if (hp > maxHp)
            {
                hp = maxHp;
            }
    ...
    public void Attack3()
    {
        if ((turn) && (life))
        {
            txt.text = "Каспер использует все мощности системы для нанесения сокрушающего урона!";
            enemy.GetComponent<Enemy>().TakeDamage(damage * 2 * dd * lvl * complexity);
            dd = 1;
            enemy.GetComponent<Enemy>().Turn();
            TakeDamage(10 * lvl);
