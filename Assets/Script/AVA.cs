...
public class AVA : MonoBehaviour
{
    ...
    private string name = "ѕроект A.V.A.";
    private int dd = 1;
    ...
    public void Attack1()
    {
        if ((turn) && (life))
        {
            txt.text = "A.V.A. боитс€, но пытаетс€ удалить вирус с устройства!";
    ...
    public void Attack2()
    {
        if ((turn) && (life))
        {
            txt.text = "Ќапуганный A.V.A. ищет у€звимость в системе вируса!";
            dd = 2;
            enemy.GetComponent<Enemy>().Turn();
            // SwapTurn.Invoke();
            //Turn();
    ...
    public void Attack3()
    {
        if ((turn) && (life))
        {
            txt.text = "A.V.A. атакует не раздумыва€... бегите, глупцы!";
            if (Random.Range(0, 2) >= 1)
            {
                enemy.GetComponent<Enemy>().TakeDamage(damage * 3 * dd * lvl * complexity);
            }
            dd = 1;
            enemy.GetComponent<Enemy>().Turn();
        }
